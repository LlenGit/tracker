...
import { Plus, Trash2, CheckSquare, Loader2, Circle, CheckCircle2, Pencil } from 'lucide-react'
...
export default function ActivitiesPage() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    if (editId) {
      await apiFetch('/api/activities', { method: 'PATCH', body: JSON.stringify({ id: editId, ...payload }) })
    } else {
  ...
  const del = async (id: string) => {
    ...
    await apiFetch('/api/activities', { method: 'DELETE', body: JSON.stringify({ id }) })
    load()
  }
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
              ...
              <select className="input" value={form.category} onChange={e => set('category', e.target.value)}>
                ...
                <option>Meeting</option><option>Site Work</option><option>Admin</option>
                <option>Follow-up</option><option>Report</option><option>Training</option><option>Other</option>
              </select>
            </div>
            <div>
            ...
            <div><label className="label">Completed Date</label><input type="date" className="input" value={form.completed_date} onChange={e => set('completed_date', e.target.value)} /></div>
